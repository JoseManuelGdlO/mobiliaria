...
async function getReports(id: number, months: string) {
    ...
    perfLog('reports.getReports', startedAt);
    return {
        data: {
            mostRent,
            eventos: {
...
async function getAggregateByWhere(id: number, whereDateSql: string, dateParams: any[]): Promise<FinancialAggregate> {
    ...
    const [incomeRows, expenseRows, byCategory, byType]: any[] = await Promise.all([
        db.query(
...
async function getFinancialSummary(
    ...
    const [currentAggregate, compareAggregate] = await Promise.all([
