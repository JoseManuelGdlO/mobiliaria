...
function getReports(id, months) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const [rows, rows2, rows3, salesRows, topClientsRows, recurrentClientsRows, topEventTypesRows,] = yield Promise.all([
        ...
        WHERE
            i.id_empresa = ${id} AND
            idm.fecha_evento >= DATE_SUB(CURDATE(), INTERVAL ${safeMonths} MONTH)
        GROUP BY
        ...
        WHERE
            ...
            em.fecha_envio_evento >= DATE_SUB(CURDATE(), INTERVAL ${safeMonths} MONTH)
        GROUP BY 
        ...
        WHERE
            id_empresa = ${id} AND
            fecha_envio_evento >= DATE_SUB(CURDATE(), INTERVAL ${safeMonths} MONTH)
        GROUP BY 
            nombre_titular_evento
        ORDER BY
            veces_agrupado DESC
        ...
        LIMIT 5`),
            db_1.db.query(`SELECT COUNT(*) AS recurrent_clients
        FROM (
        ...
        (0, perfLog_1.perfLog)('reports.getReports', startedAt);
        return {
            data: {
                mostRent,
                eventos: {
...
function getAggregateByWhere(id, whereDateSql, dateParams) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const whereGastos = whereDateSql.replace(/em\.fecha_envio_evento/g, 'gm.fecha');
        const [incomeRows, expenseRows, byCategory, byType] = yield Promise.all([
            db_1.db.query(`SELECT COALESCE(SUM(pm.costo_total), 0) AS income_total
         FROM evento_mob AS em
         LEFT JOIN pagos_mob AS pm ON em.id_evento = pm.id_evento
         WHERE em.id_empresa = ?
           AND ${whereDateSql}`, params),
            db_1.db.query(`SELECT COALESCE(SUM(gm.monto), 0) AS expenses_total
         FROM gastos_mob AS gm
         WHERE gm.id_empresa = ?
           AND gm.status = 'activo'
           AND ${whereGastos}`, params),
            db_1.db.query(`SELECT gm.categoria, COALESCE(SUM(gm.monto), 0) AS total
         FROM gastos_mob AS gm
         WHERE gm.id_empresa = ?
           AND gm.status = 'activo'
           AND ${whereGastos}
         ...
         ORDER BY total DESC`, params),
            db_1.db.query(`SELECT gm.tipo, COALESCE(SUM(gm.monto), 0) AS total
         FROM gastos_mob AS gm
         WHERE gm.id_empresa = ?
           AND gm.status = 'activo'
           AND ${whereGastos}
...
function getFinancialSummary(id, months, period, month, year, compareMonth, compareYear) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const [currentAggregate, compareAggregate] = yield Promise.all([
            getAggregateByWhere(id, whereDateSql, whereDateParams),
            hasComparison
                ? getAggregateByWhere(id, 'YEAR(em.fecha_envio_evento) = ? AND MONTH(em.fecha_envio_evento) = ?', [parsedCompareYear, parsedCompareMonth])
                : Promise.resolve(null),
        ]);
        let compareData = null;
        let delta = null;
        if (hasComparison && compareAggregate) {
